...
import { whatsappOrderUrl, PHONE_1, PHONE_2, ADDRESS, HOURS } from "@/lib/whatsapp";
  ...
  return (
    <footer className="mt-20 border-t border-border/60 bg-secondary text-secondary-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 sm:grid-cols-2 md:grid-cols-3">
        <div>
          <div className="font-display text-3xl tracking-wider">{siteData.name.toUpperCase()}</div>
          <p className="mt-3 max-w-xs text-sm text-secondary-foreground/70">{siteData.tagline}. Serving Karim Park with pizzas, shawarma, wraps, burgers and deals.</p>
          <a
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-secondary-foreground/80">
            <li><a href={`tel:${PHONE_1.replace(/-/g, "")}`} className="hover:text-primary transition">📞 {PHONE_1}</a></li>
            <li><a href={`tel:${PHONE_2.replace(/-/g, "")}`} className="hover:text-primary transition">📞 {PHONE_2}</a></li>
            <li>🕒 {HOURS}</li>
            <li>🚚 Free Delivery</li>
          </ul>
        </div>
        <div>
          ...
          <p className="mt-3 text-sm text-secondary-foreground/80 leading-relaxed">{ADDRESS}</p>
        </div>
      </div>
